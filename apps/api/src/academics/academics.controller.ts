...
export class AcademicsController {
  ...
  assessments(@CurrentUserParam() user: CurrentUser, @Query() query: Record<string, string | undefined>) {
    return this.academics.assessments(user, query);
  }
  ...
  marksEntry(@CurrentUserParam() user: CurrentUser, @Param("assessmentId") assessmentId: string) {
    return this.academics.marksEntry(user, assessmentId);
  }
  ...
  @RequirePermissions(PermissionKey.MarksReview)
  ...
  reportCards(@CurrentUserParam() user: CurrentUser, @Query() query: Record<string, string | undefined>) {
    return this.academics.reportCards(user, query);
  }

  @Post("report-cards/generate")
  @RequirePermissions(PermissionKey.ReportCardsPrepare)
  ...
  @RequirePermissions(PermissionKey.ReportCardsPublish)
